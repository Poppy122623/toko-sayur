...
if (!function_exists('get_user_image')) {
    ...
    {
        $CI =& get_instance(); // Mengganti init() dengan get_instance() untuk mendapatkan instance CodeIgniter

        // Menggunakan parameter binding untuk mencegah SQL injection
        $id = $CI->db->escape(get_current_user_id());

        $query = $CI->db->query("SELECT u.*, c.* FROM users u
